...
<br><br>
<%
ResultSet rs=stmt.executeQuery("select * from login where Name='"+user+"'");
rs.next();
%>
<form name='drdo' action='BProfile' method='post'>
	<table id='pass'>
		<tr>
			<td class='tex'>Name : </td><td><input type='text' value='<%=rs.getString(4) %>' name='fname'/></td>
		</tr>
		<tr>
			<td class='tex'>Username : </td><td><input type='text' value='<%=rs.getString(1) %>' name='uname'/></td>
		</tr>
		<tr>
			<td class='tex'>Password : </td><td><input type='text' value='<%=rs.getString(2) %>' name='pass'/></td>
		</tr>
	</table>
	<br>
