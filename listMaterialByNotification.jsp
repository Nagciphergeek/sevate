...
%><tr>
		...
		<td id="bom" style='text-align:center'><input type='text' name='bom' value='<%=rs.getString(9) %>' readonly style='text-align:center;width:29px;background-color:transparent;border:0px'/></td>
	...
	{
		%>
	<td><input type='text' name='availqty' value='<%=avail %>' readonly style='width:40px;background-color:green;border:0px;text-align:center'/></td>		
	...
	%>	
		...
		<td><input type='text' id="issqt" name='issqty' style='width:30px' autocomplete='off'/></td>
		...
		<div id='<%=mit %>' class='batchS'>
			...
			while(rs1.next()){
				...
				//System.out.println("ok2"+mit);
				
				while(rs2.next()){
					//System.out.println(i+"="+rs2.getString(7));
					//System.out.println("ok2"+rs2);
		...
		<input type='hidden' name='iid' value='<%=rs.getInt(1) %>'/></td>
	</tr>
...
<table id='btns'>
	<tr><%
		...
		<th><input type='button' id="issbut" value='ISSUE' onclick='validate1()'/></th>
		<%} }%>
...
<script>
	...
	function validate1(){	document.drdo.submit();}
		...
		var a="";var count=0;	var f1="";
		$('table tr').click(function(){
			
			 a=$(this).html();
		a=a.substring(a.search('name="bom"'),a.search('name="bom"')+180);
		a=a.substring(a.search('value="'),a.search('value="')+20);
		var d=a.indexOf('"');
		var c=a.lastIndexOf('"');
		a=a.slice(d+1,c);
	...
	$('input:text[name="issqty"]').blur(function(){
		//alert(a);
		count=0;
		f1=$(this).val();
		//alert(f);
		
		if(parseInt(f1)>parseInt(a))
			{
			alert("more than bom qty");
			$('#issbut').attr('disabled',true);
			f1.focus();
			
			}
		$('#issbut').attr('disabled',false);
	});
	...
	$('input:checkbox').click(function(){
		...
		if(count>f1)
			{
			alert("BATCHID'S SELECTED INCORRECTLY");
			
			count=count-2;
			$('#watB').attr('disabled',true);
			alert()
			}
						
		if(count==f1)
			{
			$('#watB').attr('disabled',false);
			}
